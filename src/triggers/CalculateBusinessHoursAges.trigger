            ...
            if (oldCase.Status!=updatedCase.Status && updatedCase.Last_Status_Change__c!=null) {
                ...
                if (!oldCase.IsClosed) {
                    ...
                    if (closedStatusSet.contains(updatedCase.Status)) 
                    {
                        ...
                        if(updatedCase.TimeToResolveBusinessHours__c == null)
                        {
                            updatedCase.TimeToResolveBusinessHours__c = updatedCase.Time_With_Customer__c + updatedCase.Time_With_Support__c;
                        }
                    } 
                    else if(updatedCase.Status == 'Resolved')
                    {
                        updatedCase.TimeToResolveBusinessHours__c = updatedCase.Time_With_Customer__c + updatedCase.Time_With_Support__c;
                    }
                    else
                    {
                        updatedCase.TimeToResolveBusinessHours__c = null;
                    }
                }
