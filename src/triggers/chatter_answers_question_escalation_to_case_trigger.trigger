trigger chatter_answers_question_escalation_to_case_trigger on Question (after update) {
    ...
    for (Question q: Trigger.new) {
        try {
            ...
            {
                q = [select Id, Title, Body, CommunityId, createdById, createdBy.AccountId, createdBy.ContactId, createdBy.Email from Question where Id = :q.Id];
                //Contact c = [select Id, Email from Contact where Id = :q.createdBy.ContactId Limit 1];
