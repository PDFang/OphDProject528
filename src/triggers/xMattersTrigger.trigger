trigger xMattersTrigger on BMCServiceDesk__Incident__c ( before update ) {
    ...
    // If the owner changed terminate events for this incident
    boolean ownerChanged    = ( Trigger.old[0].OwnerId != Trigger.new[0].OwnerId );
    if( ownerChanged ) {
        String incidentID = Trigger.new[0].Name;
        xMattersRESTHelper.terminateEvents( configMap.get('xMhostname'), configMap.get('xMusername'), configMap.get('xMpassword'), incidentID );
        System.debug( 'Terminated events for ' + Trigger.new[0].Name );
    }
    ...
    boolean priorityChanged = ( Trigger.old[0].BMCServiceDesk__FKPriority__c != Trigger.new[0].BMCServiceDesk__FKPriority__c );
    boolean statusChanged   = ( Trigger.old[0].BMCServiceDesk__FKStatus__c   != Trigger.new[0].BMCServiceDesk__FKStatus__c );
    //  boolean statusChanged   = ( Trigger.old[0].BMCServiceDesk__Status_ID__c   != Trigger.new[0].BMCServiceDesk__Status_ID__c );
    //  boolean priorityChanged = ( Trigger.old[0].BMCServiceDesk__Priority_ID__c != Trigger.new[0].BMCServiceDesk__Priority_ID__c );
    System.debug( 'By ID: old Priority: |' + Trigger.old[0].BMCServiceDesk__Priority_ID__c + '| new: |' + Trigger.new[0].BMCServiceDesk__Priority_ID__c + '|' );
    System.debug( 'By FK: old Priority: |' + Trigger.old[0].BMCServiceDesk__FKPriority__c + '| new: |' + Trigger.new[0].BMCServiceDesk__FKPriority__c + '|' );
    ...
    if( priorityChanged )
        triggerRule = 'Priority Change';
    if( statusChanged )
        triggerRule = 'Status Change';
    if( ownerChanged ){
        if( ownerType == 'Group' )
    ...
    if( ( notifiablePriorities.contains( p[0].Name  ) ) &&
       ...
       triggerRule != null ) {
           ...
           // First terminate all events for this Incident
           // If the ownerChanged, then we already did this. 
           if( !ownerChanged ) {
               String host = endpoint.substringBefore( '/reapi/' );
               xMattersRESTHelper.terminateEvents( host, configMap.get('xMusername'), configMap.get('xMpassword'), Trigger.new[0].Name );
           }
